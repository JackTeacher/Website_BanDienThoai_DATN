...
class TrangChuController extends Controller
{
    ...
    public function indexAdmin()
    {
        ...
        $doanhThuTungNam = DonHang::join('chi_tiet_don_hangs', 'chi_tiet_don_hangs.don_hang_id', '=', 'don_hangs.ma_don_hang')
            ...
            ->limit('5')
            ->get();
        $doanhThuLoiNhuan = DonHang::join('chi_tiet_don_hangs', 'chi_tiet_don_hangs.don_hang_id', '=', 'don_hangs.ma_don_hang')
        ->where('don_hangs.trang_thai_don_hang', '=', 3)
        ->whereYear('don_hangs.ngay_tao', '=', now()->year)
        ->select(DB::raw("MONTH(don_hangs.ngay_tao) thang"), DB::raw('sum(chi_tiet_don_hangs.gia_giam * chi_tiet_don_hangs.so_luong) * 0.4 AS loinhuan'))
        ->groupBy('thang')
        ->get();

        $soDoNhanVienDaBan = DonHang::join('nhan_viens', 'nhan_viens.id', '=', 'don_hangs.tai_khoan_nhan_vien_id')
        ->where('don_hangs.trang_thai_don_hang', '=', 3)
        ->whereYear('don_hangs.ngay_tao', '=', now()->year)
        ->select(
            DB::raw("COUNT(don_hangs.ma_don_hang) soluong"), 
            DB::raw('nhan_viens.ho_ten as hoten' ), 
            DB::raw("MONTH(don_hangs.ngay_tao) thang")
        )
        ->groupBy('thang', 'hoten')
        ->get();
        $nam = date('Y');
        ...
        foreach ($danhSachThuongHieu as $tp) {
            //$tp->phan_tram = $tp->sl_sp_ban_ra / $tongSoLuongBanRa * 100;
			 $tp->phan_tram = $tongSoLuongBanRa != 0 ? ($tp->sl_sp_ban_ra / $tongSoLuongBanRa * 100) : 0;
        }
        return view('admin/index', [
            ...
            'doanhThuLoiNhuan' => $doanhThuLoiNhuan,
            'soDoNhanVienDaBan' => $soDoNhanVienDaBan,
            'nam' => $nam,
        ]);
    }
