...
@section('content')
        ...
        <div class="page-breadcrumb">
            <div class="row">
                <div class="col-12 d-flex no-block align-items-center">
                    <h4 class="page-title">NQ Mobile: Quản trị hệ thống</h4>
                </div>
            </div>
        </div>
        <!-- ============================================================== -->
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <div id="column-chart2" style="height: 300px; width: 100%;">
                                    </div>
                                    <div id="column-chart3" style="height: 300px; width: 100%;">
                                    </div>
                                </div>
                                {{-- <div class="col-lg-3">
                                    <div class="row">
    ...
    <script type="text/javascript">
        window.onload = function() {
            ...
            var loiNhuanTemp = <?php echo json_encode($doanhThuLoiNhuan); ?>;
            var soLuongDonDaBan = <?php echo json_encode($soDoNhanVienDaBan); ?>;
            const transformedData = {};
            var loiNhuanTamThoi = loiNhuanTemp.map((value) => {
                return {
                    y: value.loinhuan,
            ...
            soLuongDonDaBan.forEach((item) => {
                const hoten = decodeURIComponent(JSON.parse('"' + item.hoten.replace(/\"/g, '\\"') + '"'));
                const dataPoint = {
                    x: item.thang,
                    y: item.soluong
                }

                if (!transformedData[hoten]) {
                    transformedData[hoten] = [];
                }

                transformedData[hoten].push(dataPoint)
            })

            const result = Object.keys(transformedData).map(hoten => ({
                hoten: hoten,
                dataPoints: transformedData[hoten]
            }));
            console.log(result);

            const soLuongDaBanDataPoints = result.map((item) => {
                return {
                    type: "line",
                    name: item.hoten,
                    showInLegend: true,
            ...
            // var loiNhuanMonth = {}
            // loiNhuanTemp.forEach((value) => {
            //     loiNhuanMonth[value.thang] = value.loinhuan
            // })
            // console.log(loiNhuanMonth);
            // var loiNhuanDataPoint = [];
            // for (let m = 1; m <= 12; m++) {
            //    const salary = loiNhuanMonth[m] 
            //    if (salary === undefined)
            //    {
            //     loiNhuanDataPoint.push({y: 0, label: "Tháng " + m });
            //    }
            // //    } else {
            // //     loiNhuanDataPoint.push({y: loiNhuanMonth[m] , label: "Tháng " + m });
            // //    }
            // }

            // console.log(loiNhuanDataPoint);
            ...
            var chart = new CanvasJS.Chart("column-chart", {
                ...
                title: {
                    text: "Doanh thu theo từng tháng năm " + <?php echo json_encode($nam); ?>,
            ...
            chart.render();
            var chart2 = new CanvasJS.Chart("column-chart2", {
                exportEnabled: true,
                animationEnabled: true,
                theme: "light2", // "light1", "light2", "dark1", "dark2"
                title: {
                    text: "Lợi nhuận theo từng tháng vào " + <?php echo json_encode($nam); ?>,
                    fontFamily: "Arial, Helvetica, sans-serif",
                },
                axisY: {
                    // title: "Doanh thu (VNĐ)",
                ...
                data: [{
                    ...
                    legendMarkerColor: "grey",
                    // legendText: "VNĐ = Việt Nam Đồng",
                    dataPoints: loiNhuanTamThoi,
                    toolTipContent: "Tháng {label} có lợi nhuận là {y} VNĐ"
                }]
            });
            chart2.render();
            //Biểu đồ tròn
            ...
            var chart3 = new CanvasJS.Chart("column-chart3", {
                title: {
                    text: "Số lượng đơn nhân viên đã bán",
                    fontFamily: "Arial, Helvetica, sans-serif",
                },
                axisX: {
                   prefix: "Tháng "
